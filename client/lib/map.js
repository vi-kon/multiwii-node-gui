...
createMap = function (id) {
    ...
    GoogleMaps.init(
        ...
        function () {
            var options, controlPanel, btnGroup, btnCenterHome, btnCenterCopter, btnTrackCopter, multicopterIcon;
            ...
            btnCenterHome = $('<div class="btn btn-sm btn-default"/>')
                .append($('<i class="icon-io-location"></i>'))
                .append(' Center Home')
                .click(geoLocateHome);

            btnCenterCopter = $('<div class="btn btn-sm btn-default"/>')
                .append($('<i class="icon-io-quad"></i>'))
                .append(' Center Copter')
                .click(geoLocateCopter);

            btnTrackCopter = $('<div class="btn btn-sm btn-default"/>')
                .append($('<i class="icon-io-compass"></i>'))
                .append(' Track Copter');

            btnGroup = $('<div class="btn-group"/>')
                .append(btnCenterHome)
                .append(btnCenterCopter)
                .append(btnTrackCopter);
            ...
            self.map.controls[google.maps.ControlPosition.TOP_LEFT].push(controlPanel[0]);

            Tracker.autorun(function () {
                if (Session.get('mspConnectedDeviceName') && Session.get('mspData') && Session.get('mspData').rawGps.fix) {
                    btnCenterCopter.removeAttr('disabled');
                    btnTrackCopter.removeAttr('disabled');
                } else {
                    btnCenterCopter.attr('disabled', 'disabled');
                    btnTrackCopter.attr('disabled', 'disabled');
                }
            });
        }
    );
